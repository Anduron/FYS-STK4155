...
// 
          // r[i] = this->distance(otherPlanet[i]);
          // if (r[i] != 0) {
                // double otherMass = otherPlanet[i].mass;
                // vec currentPos1 = this->position;
                // vec otherPos = otherPlanet[i].position;
                // F[i] += -G*otherMass*currentMass*(currentPos1 - otherPos)/pow(r[i],3);
          // }
          // else{
                // F = vec({0,0,0});
          // }
