...
void metropolis_alg(int n_Spin, int s_MC, int n_MC, double Tmp, vec &Exp_Val, vec &Tot_vals, vec &Enr_vec, map<double,double>accF)
{
  random_device rd;
...
void Results(int n_Spin, int n_MC, double Tmp, vec &Tot_vals){
  double n_factor = 1.0/((double) (n_MC-100));
...
{
int n_Spin, n_MC, my_rank, numprocs;
...
//metropolis_alg(n_Spin,n_MC,temp,Exp_Val,Enr_vec,accF);
//Results(n_Spin,n_MC,temp,Exp_Val);
...
for(double temp = T_s; temp <= T_f; temp += T_dt){
  vec Exp_Val = zeros<vec>(5);
  map <double,double> accF = Flip(temp);
  metropolis_alg(n_Spin,beginloop,endloop,temp,Exp_Val,Tot_vals,Enr_vec,accF);
  if (my_rank == 0){
    Results(n_Spin,endloop,temp,Exp_Val);
  }
